...
print(' ---------------------------')
print(' Update Fits Header for Science Target')
print(' ---------------------------')
f.write(' ---------------------------\n')
f.write(' Update Fits Header for Science Target\n')
f.write(' ---------------------------\n')

cmd_search_file_sci="find ./"+folder+"/wchen/ | cut -d / -f1-4| grep 'fits\|fts'  |sort -t'@' -k2"
print(cmd_search_file_sci)
f.write(cmd_search_file_sci+'\n')
list_file_sci=os.popen(cmd_search_file_sci,"r").read().splitlines()
#print('...calibrating science targets...')
#print('find files:',list_file_sci)
n_target=len(list_file_sci)
info_target='found '+str(n_target)+' Target\n'
print(info_target)
f.write(info_target+'\n')

list_filename_sci=[i.split('/',-1)[-1] for i in list_file_sci]
#print(list_filename_sci)


list_sci_1=[i[9:-4] for i in list_filename_sci]
list_sci_2=[i.split('eden',1)[0] for i in list_sci_1]
list_sci_name=[i[0:-4] for i in list_sci_2]
#print(list_sci_name)

'''
list_sci_name_uniq=[]
for i in list_sci_name:
    if i not in list_sci_name_uniq:
        list_sci_name_uniq.append(i)

print(list_sci_name_uniq)
sci_name=list_sci_name_uniq[0].upper()
print('Science Target: ',sci_name)
'''
#sys.exit(0)

#cmd_target_name="find ./"+folder+"/ | grep wchen | grep fits | cut -d / -f4 | cut -d - -f2 | sort | uniq"
#print('target: ', cmd_target_name)
#target_name=os.popen(cmd_target_name,"r").read().splitlines()[0]
#print(target_name)
#filename_sci_cap=filename_sci.capitalize()
#print('Science Target: ',filename_sci_cap)
#sys.exit(0)
...
#    print(k,'[TYPE]',imgtype,'[OBJ]',obj_name,'[RA]',obj_ra,'[Dec]', obj_dec,'[FIL]',filter_name,'[EXPTIME]',exptime, '[Date]',date,'[Time]', time)   
    info_target=str(k)+'/'+str(n_target)+' '+str(filename)+' [TYPE] '+str(imgtype)+' [OBJ] '+str(obj_name)+' [RA] '+str(obj_ra)+' [Dec] '+str(obj_dec)+' [FIL] '+str(filter_name)+' [EXPTIME] '+str(exptime)+' [Date] '+str(date)+' [Time] '+str(time)
    print(info_target)
    f.write(info_target+'\n')
...
'''
n_totalfile=n_biasdark+n_flat+n_target
info_totalfile='total found '+str(n_totalfile)+' files'
print(info_totalfile)
f.write(info_totalfile+'\n')
'''

'''
for i in list_updated_fits:
    cmd_mv_file="mv "+i+" ./"+dir_mod
    os.popen(cmd_mv_file,"r")
'''
